...
import QuestionsSection from "./_components/question-section";
...
function VideoPlayer({
  ...
  const [playedSceonds, setPlayedSeconds] = useState(0);
      ...
      {showCenterControl && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none animate-fade-in-out">
          <div className="bg-black/60 rounded-full p-6 backdrop-blur-sm">
            {playing ? (
              <PauseIcon className="w-20 h-20 text-white" strokeWidth={1.5} />
            ) : (
              <PlayIcon className="w-20 h-20 text-white" strokeWidth={1.5} />
            )}
          </div>
        </div>
      )}
